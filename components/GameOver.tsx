...
import { RefreshCw, Home, Trophy, Award, ShoppingBag } from 'lucide-react';
...
const GameOver: React.FC<GameOverProps> = ({ session, onRestart, onHome, onOpenShop }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full w-full px-4 animate-fade-in py-8">
      ...
      <div className="w-full max-w-sm space-y-3 mt-auto">
        ...
        <button
          onClick={onOpenShop}
          className="w-full py-3 bg-slate-800/50 border border-slate-700 text-yellow-500 hover:bg-slate-800 rounded-2xl font-bold flex items-center justify-center transition-colors active:scale-[0.98]"
        >
