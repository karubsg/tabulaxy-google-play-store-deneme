...
const GameSetup: React.FC<GameSetupProps> = ({ mode, onStart, onBack }) => {
  ...
  // Default: All categories enabled (except Mixed=6 usually, or include all 1-5)
  // Categories: 1=Entertainment, 2=Science, 3=Daily, 4=Culture, 5=Tech
  const [selectedCategories, setSelectedCategories] = useState<number[]>([1, 2, 3, 4, 5]);
  ...
  return (
    <div className="w-full h-full flex flex-col animate-fade-in pb-8">
      ...
      <div className="flex-1 space-y-8 overflow-y-auto scrollbar-hide px-1">
        ...
        {/* Category Selection */}
        <div className="space-y-3">
          <label className="flex items-center gap-2 text-cyan-400 font-bold text-xs uppercase tracking-wider">
            <Map size={14} /> Kategoriler
          </label>
          <div className="grid grid-cols-3 gap-2">
            {[
              { id: 1, name: 'Eğlence', icon: '🎬', color: 'bg-red-500' },
              { id: 2, name: 'Bilim', icon: '🔬', color: 'bg-teal-500' },
              { id: 3, name: 'Günlük', icon: '🏠', color: 'bg-blue-500' },
              { id: 4, name: 'Kültür', icon: '🎭', color: 'bg-emerald-500' },
              { id: 5, name: 'Teknoloji', icon: '💻', color: 'bg-purple-500' },
            ].map(cat => {
              ...
              return (
                <button
                  ...
                  className={`p-2 rounded-xl border transition-all flex flex-col items-center justify-center gap-1 relative overflow-hidden ${isSelected ? `${cat.color}/20 border-white/40 text-white shadow-lg shadow-${cat.color}/20` : 'bg-slate-800 border-slate-700 text-slate-500 grayscale'}`}
                >
                  <span className="text-xl">{cat.icon}</span>
                  <span className="font-bold text-[10px] text-center">{cat.name}</span>
                  {isSelected && (
