...
import { wordService, fetchTabuCards } from './services/WordService';
...
const App: React.FC = () => {
    ...
    const handleStartGame = async (settings: GameSettings) => {
        ...
        if (selectedMode !== GameMode.JOURNEY) {
            ...
            // Preload words into RAM for zero-latency gameplay
            await wordService.preloadForMode(selectedMode, settings.enabledCategories);

            const cards = await fetchTabuCards(selectedMode, [], INITIAL_BATCH_SIZE, settings.enabledCategories);
            if (cards.length > 0) {
                setCardQueue(cards);
    ...
    const refillQueueIfNeeded = useCallback(async (currentQueueLength: number, mode: GameMode, currentUsedWords: string[]) => {
        if (currentQueueLength <= REFILL_THRESHOLD && !isFetchingRef.current) {
            ...
            setIsRefilling(true);
            try {
                const newCards = await fetchTabuCards(mode, currentUsedWords, REFILL_BATCH_SIZE, session.settings.enabledCategories);
    ...
    const loadNextCard = async (overrideMode?: GameMode) => {
        ...
        if (cardQueue.length > 0) {
            ...
            setIsLoading(true);
            const newCards = await fetchTabuCards(modeToUse, usedWords, 5, session.settings.enabledCategories);
    ...
    return (
        ...
        <div className="h-dvh w-full bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-800 via-slate-950 to-black text-slate-100 flex flex-col overflow-hidden font-outfit relative">
            ...
            <main className="flex-1 min-h-0 w-full max-w-lg mx-auto p-4 flex flex-col items-center justify-center relative">
                ...
                {gameState === GameState.MENU && <Menu onSelectMode={handleSelectMode} onOpenShop={() => setIsShopOpen(true)} />}
                ...
                {gameState === GameState.GAME_OVER && (
                    <GameOver
                        session={session}
                        onRestart={() => handleStartGame(session.settings)}
